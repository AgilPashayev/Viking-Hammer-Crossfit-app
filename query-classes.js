...
async function queryClasses() {
  ...
  const { data, error } = await supabase
    .from('classes')
    .select(
      'id, name, description, duration_minutes, max_capacity, status, category, difficulty, created_at',
    )
  ...
  if (data.length === 0) {
    ...
    data.forEach((cls, index) => {
      ...
      if (cls.description) {
        const desc =
          cls.description.length > 70 ? cls.description.substring(0, 70) + '...' : cls.description;
