...
async function simulateUserFlow() {
  ...
  try {
    ...
    const markPromises = unreadAnnouncements.map(async (ann) => {
      const response = await fetch(`${API_BASE}/api/announcements/${ann.id}/mark-read`, {
        method: 'POST',
...
console.log(
  `${colors.cyan}║  ANNOUNCEMENT MARK-AS-READ - COMPLETE USER FLOW SIMULATION        ║${colors.reset}`,
);
console.log(
  `${colors.cyan}╚════════════════════════════════════════════════════════════════════╝${colors.reset}\n`,
);
