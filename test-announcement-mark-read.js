  ...
  try {
    ...
    if (allAnnouncementsData.success && Array.isArray(allAnnouncementsData.data)) {
      ...
      if (totalAnnouncements > 0) {
        ...
        log.info(`  Has read_by_users field: ${firstAnnouncement.hasOwnProperty('read_by_users') ? 'YES' : 'NO'}`);
    ...
    const unreadResponse = await fetch(`${API_BASE}/api/announcements/member?userId=${TEST_USER_ID}&unreadOnly=true`);
    ...
    if (unreadData.data && unreadData.data.length > 0) {
      ...
      const reFilteredResponse = await fetch(`${API_BASE}/api/announcements/member?userId=${TEST_USER_ID}&unreadOnly=true`);
      ...
      if (reFilteredData.success) {
        ...
        if (!stillInUnread) {
          log.success(`Announcement #${announcementToMark.id} is correctly filtered out from unread list`);
