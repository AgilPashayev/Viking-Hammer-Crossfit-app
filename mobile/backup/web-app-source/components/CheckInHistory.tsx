            ...
            <h3>
              {
                checkIns.filter((c) => {
                  ...
                  return (
                    checkInDate.toDateString() === today.toDateString() && c.role === 'instructor'
                  );
                }).length
              }
            </h3>
                ...
                <div className="filter-group">
                  ...
                  >
                    ...
                    <option value="completed">
                      ✅ {t('admin.checkInHistory.filters.battleComplete')}
                    </option>
                  </select>
                </div>
                ...
                <div className="filter-group">
                  ...
                  >
                    ...
                    <option value="Monthly">
                      {t('admin.checkInHistory.filters.monthlyWarrior')}
                    </option>
                    <option value="Monthly Unlimited">
                      {t('admin.checkInHistory.filters.unlimitedViking')}
                    </option>
        ...
        {(statusFilter !== 'all' ||
          ...
          <div className="applied-filters">
            <span className="filter-label">
              {t('admin.checkInHistory.filters.appliedFiltersLabel')}
            </span>
            {statusFilter !== 'all' && (
              <span className="filter-tag">
                {t('admin.checkInHistory.filters.statusFilter', { status: statusFilter })}
              </span>
            )}
            {membershipFilter !== 'all' && (
              <span className="filter-tag">
                {t('admin.checkInHistory.filters.typeFilter', { type: membershipFilter })}
              </span>
            )}
            {customDateRange.startDate && (
              <span className="filter-tag">
                {t('admin.checkInHistory.filters.fromFilter', { date: customDateRange.startDate })}
              </span>
            )}
            {customDateRange.endDate && (
              <span className="filter-tag">
                {t('admin.checkInHistory.filters.toFilter', { date: customDateRange.endDate })}
              </span>
            )}
          </div>
        )}
              ...
              <tbody>
                {filteredCheckIns.map((record) => {
                  ...
                  const diffMinutes = Math.floor(
                    (now.getTime() - checkInDate.getTime()) / (1000 * 60),
                  );
                  ...
                  return (
                    <tr
                      key={record.id}
                      className={`checkin-row ${isActive ? 'active' : 'completed'}`}
                    >
                      <td>
        ...
        <div className="modal-overlay" onClick={() => setShowMemberPopup(false)}>
          <div className="member-popup" onClick={(e) => e.stopPropagation()}>
            <div className="popup-header">
              ...
              <button className="close-btn" onClick={() => setShowMemberPopup(false)}>
                {t('admin.checkInHistory.memberPopup.closeButton')}
              </button>
            </div>
            ...
            <div className="popup-content">
              ...
              <div className="member-details-section">
                ...
                <div className="member-info-details">
                  ...
                  <p>
                    <strong>{t('admin.checkInHistory.memberPopup.emailLabel')}</strong>{' '}
                    {selectedMember.email}
                  </p>
                  <p>
                    <strong>{t('admin.checkInHistory.memberPopup.phoneLabel')}</strong>{' '}
                    {selectedMember.phone}
                  </p>
                  <p>
                    <strong>{t('admin.checkInHistory.memberPopup.membershipLabel')}</strong>{' '}
                    {selectedMember.membershipType}
                  </p>
                  <p>
                    <strong>{t('admin.checkInHistory.memberPopup.statusLabel')}</strong>{' '}
                    <span className={`status-badge status-${selectedMember.status}`}>
                      {t(`admin.memberManagement.status.${selectedMember.status}`)}
                    </span>
                  </p>
                  <p>
                    <strong>{t('admin.checkInHistory.memberPopup.joinedLabel')}</strong>{' '}
                    {formatDate(selectedMember.joinDate)}
                  </p>
                  <p>
                    <strong>{t('admin.checkInHistory.memberPopup.lastCheckInLabel')}</strong>{' '}
                    {selectedMember.lastCheckIn
                      ? formatDate(selectedMember.lastCheckIn)
                      : t('admin.checkInHistory.noValue')}
                  </p>
                </div>
              </div>
                ...
                <div className="stats-grid">
                  <div className="stat-card">
                    ...
                    <span className="stat-label">
                      {t('admin.checkInHistory.memberPopup.today')}
                    </span>
                  </div>
                  <div className="stat-card">
                    <span
                    ...
                    <span className="stat-label">
                      {t('admin.checkInHistory.memberPopup.thisWeek')}
                    </span>
                  </div>
                  <div className="stat-card">
                    <span
                    ...
                    <span className="stat-label">
                      {t('admin.checkInHistory.memberPopup.thisMonth')}
                    </span>
                  </div>
                  <div className="stat-card">
                    <span
                    ...
                    <span className="stat-label">
                      {t('admin.checkInHistory.memberPopup.thisYear')}
                    </span>
                  </div>
                  <div className="stat-card">
                    <span
                    ...
                    <span className="stat-label">
                      {t('admin.checkInHistory.memberPopup.allTime')}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="history-list">
                      ...
                      <div key={checkIn.id} className="history-item">
                        <div className="history-time">
                          ...
                          <span className="history-time-value">
                            {formatBakuTime(checkIn.checkInTime)}
                          </span>
                        </div>
                  ...
                  {memberStats.history.length > 10 && (
                    <p className="more-history">
                      {t('admin.checkInHistory.memberPopup.showingRecent', {
